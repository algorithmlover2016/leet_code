...
public:
    void solveSudoku(std::vector<std::vector<char>> & board) {
        // plagiarizing from https://leetcode.com/problems/sudoku-solver/discuss/15752/Straight-Forward-Java-Solution-Using-Backtracking
    ...
    bool isValid(std::vector<std::vector<char>> & board,
        ...
        for (int index = 0; index < ELEMENTS.size(); index++) {
            ...
            // check the 3 * 3 block
            // 3 * (row [col] / 3) will floor row [col] to 0, 3, 6.
            // when index from 0 to 9, [row[0, 3, 6] + 0][col[0, 3, 6] + [0, 1 ,2]]
            // [row[0, 3, 6] + 1][col[0, 3, 6] + [0, 1 ,2]]
            // [row[0, 3, 6] + 2][col[0, 3, 6] + [0, 1 ,2]]
            if (board[3 * (row / 3) + index / 3][3 * (col / 3) + index % 3] == c) {
                return false;
            }
        }
        return true;
...
private:
        static std::string const ELEMENTS;
};

std::string const SolutionBruteFs::ELEMENTS("123456789");

class Solution {
private:
    // encapsulates a single cell on a Sudoku board
	struct cell {
...
private:
	...
	// once there's one possibility left the function recurses back into setVal()
	bool updateConstraints(int i, int j, int excludedValue) {
        ...
        // have to set number at i, j
		for (int v = 1; v <= 9; v++) {
			if (!c.constraints[v]) {
                // there is only one value that meets the condition
				return setVal(i, j, v);
			}
		}
...
public:
	void solveSudoku(std::vector<std::vector<char>> &board) {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				if (board[i][j] != '.' && !setVal(i, j, board[i][j] - '0')) {
